...
def train(args):
    ...
    #        input = fluid.data(name='image', shape=(None,1,24,224,224), dtype='float32')
            input = fluid.data(name='image', shape=(None,1,model_para['sample']['seglen']*3,train_config.TRAIN.target_size,train_config.TRAIN.target_size), dtype='float32')
    ...
    with fluid.program_guard(valid_prog, startup):
        with fluid.unique_name.guard():
      #      input = fluid.data(name='image', shape=(None, 1, 24, 224, 224), dtype='float32')
            input = fluid.data(name='image', shape=(None,1,model_para['sample']['seglen']*3,valid_config.VALID.target_size,valid_config.VALID.target_size), dtype='float32')
            label = fluid.data(name='label', shape=(None, 1), dtype='int64')
